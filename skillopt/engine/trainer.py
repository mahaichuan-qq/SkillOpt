...
def _save_skill(out_root: str, step: int, content: str) -> None:
    ...
    with open(
        os.path.join(skills_dir, f"skill_v{step:04d}.md"), "w", encoding="utf-8"
    ) as f:
        f.write(content)
...
class ReflACTTrainer:
    ...
    def train(self) -> dict:
        ...
        configure_qwen_chat(
            ...
            stream=cfg.get("qwen_chat_stream"),
            ...
            optimizer_stream=cfg.get("optimizer_qwen_chat_stream"),
            ...
            target_stream=cfg.get("target_qwen_chat_stream"),
        )
        configure_minimax_chat(
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for step_in_epoch in range(steps_per_epoch):
                ...
                if not all_failure_patches and not all_success_patches:
                    ...
                    with open(
                        os.path.join(step_dir, "step_record.json"), "w", encoding="utf-8"
                    ) as f:
                ...
                with open(
                    os.path.join(step_dir, "merged_patch.json"), "w", encoding="utf-8"
                ) as f:
                ...
                if is_full_rewrite_minibatch_mode(update_mode):
                    ...
                    with open(
                        os.path.join(step_dir, "ranked_edits.json"), "w", encoding="utf-8"
                    ) as f:
                    ...
                    if lr_control_mode == "autonomous":
                        ...
                        with open(
                            os.path.join(step_dir, "lr_decision.json"), "w", encoding="utf-8"
                        ) as f:
                            json.dump(lr_decision, f, ensure_ascii=False, indent=2)
                        with open(
                            os.path.join(out_root, "lr_history.jsonl"), "a", encoding="utf-8"
                        ) as f:
                            f.write(json.dumps({
                                "step": global_step,
                                "epoch": epoch,
                ...
                if update_mode == "rewrite_from_suggestions":
                    ...
                    if rewrite_result and rewrite_result.get("new_skill"):
                        ...
                        apply_report = []
                        with open(
                            os.path.join(step_dir, "rewrite_result.json"), "w", encoding="utf-8"
                        ) as f:
                    ...
                    if selected_candidate:
                        ...
                        with open(
                            os.path.join(step_dir, "full_rewrite_result.json"),
                            "w",
                            encoding="utf-8",
                        ) as f:
                            json.dump(
                ...
                with open(
                    os.path.join(step_dir, "candidate_skill.md"), "w", encoding="utf-8"
                ) as f:
                    f.write(candidate_skill)
                if apply_report:
                    with open(
                        os.path.join(step_dir, "edit_apply_report.json"), "w", encoding="utf-8"
                    ) as f:
            ...
            if use_slow:
                ...
                if os.path.exists(slow_done_path):
                    ...
                    _save_skill(out_root, global_step, current_skill)
                    with open(
                        os.path.join(out_root, "best_skill.md"), "w", encoding="utf-8"
                    ) as f:
                        f.write(best_skill)
                    ...
                    if slow_result and slow_result.get("slow_update_content"):
                        ...
                        with open(
                            os.path.join(slow_dir, "candidate_skill.md"),
                            "w",
                            encoding="utf-8",
                        ) as f:
        ...
        if cfg["eval_test"]:
            ...
            try:
                if skill_hash(current_skill) == skill_hash(best_skill):
                    ...
                    if final_gate_score > best_score:
                        ...
                        best_origin = current_origin
                        with open(
                            os.path.join(out_root, "best_skill.md"),
                            "w",
                            encoding="utf-8",
                        ) as f:
                            f.write(best_skill)
            ...
            with open(
                os.path.join(baseline_test_dir, "summary.json"), "w", encoding="utf-8"
            ) as f:
                json.dump(
                    {
                        k: {
            ...
            try:
                if skill_hash(current_skill) == skill_hash(best_skill):
                    ...
                    with open(
                        os.path.join(final_test_dir, "summary.json"), "w", encoding="utf-8"
                    ) as f:
                        json.dump(
                            {
                                k: {
